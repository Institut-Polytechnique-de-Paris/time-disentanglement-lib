...
class Res12_Quadratic(nn.Module):
    def __init__(self,inchan, channel, L, dim,hw,normalize=False,AF=None):
        ...
        self.ln1 = nn.Linear(8*dim*channel*L, 1)
        self.ln2 = nn.Linear(8*dim*channel*L, 1)
        self.lq = nn.Linear(8*dim*channel*L, 1)
        self.Square = Square()
    ...
    def forward(self, x_in):
        ...
        output = output.view(x_in.shape[0], -1) #view(-1, int(self.hw/8)*int(self.hw/8)*8*self.dim)
